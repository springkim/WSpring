...
::  WSpring
::
::  Created by kimbomm on 2018. 08. 29...
...
title install_vim
echo Downloading...
curlw -L "https://github.com/springkim/WSpring/releases/download/program/vim.zip" -o "%TEMP%\vim.zip"

echo Unzipping...
call :SafeRMDIR "%UserProfile%\hddb"
powershell -nologo -noprofile -command "& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::ExtractToDirectory('%TEMP%\vim.zip', '%SystemDrive%\vim'); }"

call :DownloadSetw
setw "%SystemDrive%\vim"
