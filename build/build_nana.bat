...
powershell "$wc = New-Object System.Net.WebClient;$html=$wc.DownloadString('http://nanapro.org/en-us/');add-type -Path %SystemRoot%\System32\HtmlAgilityPack.dll;$doc = New-Object HtmlAgilityPack.HtmlDocument;$doc.LoadHtml($html);$doc.DocumentNode.SelectSingleNode('html').SelectSingleNode('body').SelectSingleNode('div[3]').SelectSingleNode('div[2]').SelectSingleNode('div').SelectSingleNode('div').SelectSingleNode('span[2]').InnerText | out-file -encoding ascii nanaver.txt"
